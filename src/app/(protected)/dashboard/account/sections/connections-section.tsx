"use client";

import { useState, useEffect, useCallback } from "react";
import { useWorkspace } from "@/contexts/team-context";
import { useTranslation } from "@/lib/i18n";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Badge } from "@/components/ui/badge";
import { Separator } from "@/components/ui/separator";
import { InviteCodeDisplay } from "@/components/team/invite-code-display";
import { Link2, Plus, Check, X } from "lucide-react";
import { toast } from "sonner";

interface ConnectionData {
  id: string;
  status: string;
  requester_team_id: string;
  target_team_id: string;
  requester: { id: string; name: string };
  target: { id: string; name: string };
}

export function ConnectionsSection() {
  const { t } = useTranslation();
  const { currentWorkspace, can } = useWorkspace();
  const [connections, setConnections] = useState<ConnectionData[]>([]);
  const [connectCode, setConnectCode] = useState("");
  const [connecting, setConnecting] = useState(false);

  const fetchConnections = useCallback(async () => {
    if (!currentWorkspace) return;
    const res = await fetch(`/api/teams/${currentWorkspace.id}/connections`);
    const json = await res.json();
    if (json.success) setConnections(json.data || []);
  }, [currentWorkspace]);

  useEffect(() => { fetchConnections(); }, [fetchConnections]);

  const handleConnect = async () => {
    if (!currentWorkspace || !connectCode.trim()) return;
    setConnecting(true);
    try {
      const res = await fetch(`/api/teams/${currentWorkspace.id}/connections`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ connection_code: connectCode.trim() }),
      });
      const json = await res.json();
      if (json.success) {
        toast.success(t("team.connections.requestSent"));
        setConnectCode("");
        fetchConnections();
      } else {
        toast.error(json.error || t("common.failed"));
      }
    } catch {
      toast.error(t("common.error"));
    } finally {
      setConnecting(false);
    }
  };

  const handleRespond = async (connId: string, status: "accepted" | "rejected") => {
    if (!currentWorkspace) return;
    const res = await fetch(`/api/teams/${currentWorkspace.id}/connections/${connId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ status }),
    });
    const json = await res.json();
    if (json.success) {
      toast.success(status === "accepted" ? t("team.connections.accepted") : t("team.connections.rejected"));
      fetchConnections();
    } else {
      toast.error(json.error || t("common.failed"));
    }
  };

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold">{t("team.connections.title")}</h2>
        <p className="text-sm text-muted-foreground mt-1">{t("team.connections.description")}</p>
      </div>
      <Separator />

      {can("team_settings.manage") && currentWorkspace && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">{t("team.connections.yourCode")}</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-muted-foreground mb-4">{t("team.connections.yourCodeDescription")}</p>
            <InviteCodeDisplay code={currentWorkspace.inviteCode} teamId={currentWorkspace.id} />
          </CardContent>
        </Card>
      )}

      {can("team_settings.manage") && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">{t("team.connections.connectTo")}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex gap-2">
              <Input
                value={connectCode}
                onChange={(e) => setConnectCode(e.target.value)}
                placeholder={t("team.connections.enterCode")}
              />
              <Button onClick={handleConnect} disabled={connecting || !connectCode.trim()}>
                <Plus className="size-4 mr-2" />
                {connecting ? t("team.connections.connecting") : t("team.connections.connect")}
              </Button>
            </div>
          </CardContent>
        </Card>
      )}

      <div className="space-y-2">
        {connections.length === 0 && (
          <div className="text-center py-10 text-muted-foreground">
            <Link2 className="size-8 mx-auto mb-2 opacity-40" />
            <p className="text-sm">{t("team.connections.noConnections")}</p>
          </div>
        )}
        {connections.map((conn) => {
          const otherTeam = conn.requester_team_id === currentWorkspace?.id ? conn.target : conn.requester;
          const isIncoming = conn.target_team_id === currentWorkspace?.id;

          return (
            <Card key={conn.id}>
              <CardContent className="flex items-center gap-4 py-4">
                <Link2 className="size-5 text-muted-foreground" />
                <div className="flex-1">
                  <span className="font-medium text-sm">{otherTeam.name}</span>
                  <span className="text-xs text-muted-foreground ml-2">
                    {isIncoming ? t("team.connections.incoming") : t("team.connections.outgoing")}
                  </span>
                </div>
                <Badge variant={
                  conn.status === "accepted" ? "default" :
                  conn.status === "rejected" ? "destructive" : "secondary"
                }>
                  {conn.status}
                </Badge>
                {isIncoming && conn.status === "pending" && can("team_settings.manage") && (
                  <div className="flex gap-1">
                    <Button size="icon" variant="ghost" className="size-8 text-green-600" onClick={() => handleRespond(conn.id, "accepted")}>
                      <Check className="size-4" />
                    </Button>
                    <Button size="icon" variant="ghost" className="size-8 text-destructive" onClick={() => handleRespond(conn.id, "rejected")}>
                      <X className="size-4" />
                    </Button>
                  </div>
                )}
              </CardContent>
            </Card>
          );
        })}
      </div>
    </div>
  );
}
