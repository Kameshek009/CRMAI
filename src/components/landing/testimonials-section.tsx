    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 -z-10">
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse, rgba(244,114,182,0.05) 0%, rgba(167,139,250,0.03) 40%, transparent 70%)",
            filter: "blur(100px)",
          }}
        />
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
          {testimonials.map((t, i) => (
            <motion.div
              ...
              whileHover={{ y: -4 }}
              className="group relative rounded-3xl border border-white/10 dark:border-white/10 bg-white/[0.03] dark:bg-white/[0.03] backdrop-blur-xl p-6 sm:p-7 hover:border-white/20 dark:hover:border-white/20 transition-all duration-300 hover:shadow-2xl hover:shadow-purple-500/5"
            >
              {/* Background glow on hover */}
              <div
                className={`pointer-events-none absolute inset-0 rounded-3xl bg-gradient-to-br ${t.gradient} opacity-0 group-hover:opacity-[0.06] transition-opacity duration-500`}
              />
              ...
              <div className="flex items-center gap-3 pt-5 border-t border-white/10 dark:border-white/10">
                <div
                  className={`w-10 h-10 rounded-full bg-gradient-to-br ${t.gradient} flex items-center justify-center text-xs font-bold text-white shrink-0 shadow-lg`}
                >
                  {t.avatar}
                </div>
                <div>
